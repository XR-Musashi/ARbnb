...
export const api = {
  properties: {
    ...
    uploadModel: async (id, file) => {
      ...
      const res = await fetch(`${BASE}/api/properties/${id}/model`, {
        method: 'POST',
    ...
    uploadFloorPlan: async (id, file) => {
      ...
      const form = new FormData();
      form.append('file', file);
      const res = await fetch(`${BASE}/api/properties/${id}/floor-plan`, {
        method: 'POST',
        headers: token ? { Authorization: `Bearer ${token}` } : {},
        body: form,
      });
  ...
  sessions: {
    create: (propertyId, durationMs) =>
      request('POST', '/api/sessions', { propertyId, durationMs }),
